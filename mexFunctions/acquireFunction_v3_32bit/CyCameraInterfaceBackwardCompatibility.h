...
public:
    CAMERA_BACKWARD_COMPATIBILITY( SizeX, CY_CAMERA_PARAM_SIZE_X, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( SizeY, CY_CAMERA_PARAM_SIZE_Y, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( OffsetX, CY_CAMERA_PARAM_OFFSET_X, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( OffsetY, CY_CAMERA_PARAM_OFFSET_Y, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( DecimationX, CY_CAMERA_PARAM_DECIMATION_X, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( DecimationY, CY_CAMERA_PARAM_DECIMATION_Y, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( DecimationBlockX, CY_CAMERA_PARAM_DECIMATION_BLOCK_X, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( DecimationBlockY, CY_CAMERA_PARAM_DECIMATION_BLOCK_Y, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( BinningX, CY_CAMERA_PARAM_BINNING_X, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( BinningY, CY_CAMERA_PARAM_BINNING_Y, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( PixelType,   CY_CAMERA_PARAM_PIXEL_TYPE, __int64 );
    CAMERA_BACKWARD_COMPATIBILITY( TapQuantity, CY_CAMERA_PARAM_TAP_QUANTITY, unsigned char );
    CAMERA_BACKWARD_COMPATIBILITY( ImageType, CY_CAMERA_PARAM_IMAGE_TYPE, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( SynchronizationMode, CY_CAMERA_PARAM_SYNCHRO_MODE, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( ExposureTime, CY_CAMERA_PARAM_EXPOSURE_TIME, unsigned long );
    CAMERA_BACKWARD_COMPATIBILITY( FrameRate,    CY_CAMERA_PARAM_FRAME_RATE,    double );
    CAMERA_BACKWARD_COMPATIBILITY( DigitalShift, CY_CAMERA_PARAM_DIGITAL_SHIFT, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( TestPattern,  CY_CAMERA_PARAM_TEST_PATTERN,  unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY( ImageFlip,    CY_CAMERA_PARAM_IMAGE_FLIP,    bool );
    CAMERA_BACKWARD_COMPATIBILITY( BlackLevel,   CY_CAMERA_PARAM_BLACK_LEVEL,   unsigned short );
    ...
    CAMERA_BACKWARD_COMPATIBILITY_GET( SizeX, CY_CAMERA_PARAM_SIZE_X, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY_GET( SizeY, CY_CAMERA_PARAM_SIZE_Y, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY_GET( OffsetX, CY_CAMERA_PARAM_OFFSET_X, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY_GET( OffsetY, CY_CAMERA_PARAM_OFFSET_Y, unsigned short );
    ...
    CyResult Grab( const CyChannel& aChannel, CyImageBuffer&   aBuf, unsigned long    aFlags )
        { return GetGrabber().Grab( aChannel, aBuf, aFlags ); }
	CyResult StartGrabbing( const CyChannel& aChannel, CyImageBuffer&   aBuf, unsigned long    aFlags = 0 )
        { return GetGrabber().StartGrabbing( aChannel, aBuf, aFlags ); }
	CyResult StopGrabbing( const CyChannel& aChannel )
        { return GetGrabber().StopGrabbing( aChannel ); }
    bool IsStarted( const CyChannel& aChannel )
        { return GetGrabber().IsStarted( aChannel ); }
    bool IsStopped( const CyChannel& aChannel )
        { return GetGrabber().IsStopped( aChannel ); }

    // Gains & offsets
    CAMERA_BACKWARD_COMPATIBILITY_GAIN    ( Gain, CY_CAMERA_PARAM_GAIN, long );
    CAMERA_BACKWARD_COMPATIBILITY_GET_GAIN( Gain, CY_CAMERA_PARAM_GAIN, short );
    CAMERA_BACKWARD_COMPATIBILITY_GET_GAIN( Gain, CY_CAMERA_PARAM_GAIN, unsigned short );
    CAMERA_BACKWARD_COMPATIBILITY_GAIN    ( Offset, CY_CAMERA_PARAM_OFFSET, long );
    CAMERA_BACKWARD_COMPATIBILITY_GET_GAIN( Offset, CY_CAMERA_PARAM_OFFSET, short );
    CAMERA_BACKWARD_COMPATIBILITY_GET_GAIN( Offset, CY_CAMERA_PARAM_OFFSET, unsigned short );
    ...
    CyResult SetStartingBayerFlag( unsigned long aFlag, bool aAdjustWithOffsets )
    {
    ...
    {
        return GetParameterULong( CY_CAMERA_PARAM_STARTING_BAYER_FLAG );
    }
    ...
    unsigned long GetPixelConversionFlags() const
    {
        return GetParameterULong( CY_CAMERA_PARAM_CONVERSION_FLAGS );
    }
    ...
    {
        aName = GetParameterName( aIndex );
        aType = GetParameterType( aIndex );
        if ( ( aName.empty() ) ||
