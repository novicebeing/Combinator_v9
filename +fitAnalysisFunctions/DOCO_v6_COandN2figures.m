function DOCO_v6_COandN2figures(fitobjnames,fitobjs)
    ...
    for ii = 1:numel(fitobjs)
        ...
        ODmean = (edouble(ODtrace(secondidx+i),ODtraceErr(secondidx+i))+edouble(ODtrace(firstidx+i),ODtraceErr(firstidx+i)))/2;
         ...
         y(ii) = (dDOCOdt2*2 + 0*dDOCOdt)/2/ODmean;
         xCO(ii) = edouble(1,0)*CO;
         xN2(ii) = edouble(1,0)*N2;
         xO3(ii) = edouble(1,0)*O3;
         xD2(ii) = edouble(1,0)*D2;
         yDOCO_O3(ii) = DOCOmean.value(:).*O3/ODmean.value(:);
         yDOCO_LOSS(ii) = DOCOmean.value(:);%*1e16/ODmean.value(:);
    ...
    X = [xCO.value(:).^2 xCO.value(:).*xN2.value(:) -yDOCO_O3*1e16];% yDOCO_LOSS*1e16];
    ...
    %plot(xfit,ci(:,1),'r--','LineWidth',1);
    %plot(xfit,ci(:,2),'r--','LineWidth',1);
    scatter(xN2,yysim,'bo','MarkerFaceColor','b','MarkerEdgeColor','b');
    set(gca,'FontSize',14);
    ...
    %plot(xfit,ci(:,1),'r--','LineWidth',1);
    %plot(xfit,ci(:,2),'r--','LineWidth',1);
    scatter(xO3,yysim,'bo','MarkerFaceColor','b','MarkerEdgeColor','b');
    set(gca,'FontSize',14);
